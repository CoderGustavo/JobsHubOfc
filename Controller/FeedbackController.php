...
class FeedbackController{
    protected $feedback, $table, $conn, $pk;

    public function __construct(){
        include_once ROOT."/Model/feedback.php";
        $this->feedback = new Feedback();
        $this->conn = $this->feedback->getConnection();
        $this->table = $this->feedback->getTable();
        $this->pk = $this->feedback->getPk();
    }
    
    public function updateInfos($userlogged, $infos, $id_feedback){
        $a = "";
        $index = 1;
        ...
        $query->bindParam(":$this->pk", $id_feedback);
        $index = 1;
        foreach ($infos as $key => $info) {
