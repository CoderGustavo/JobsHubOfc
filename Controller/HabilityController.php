...
class HabilityController{
    ...
    public function __construct(){
        include_once ROOT."/Model/hability.php";
        $this->hability = new Hability();
        $this->conn = $this->hability->getConnection();
        $this->table = $this->hability->getTable();
        $this->pk = $this->hability->getPk();
    }
    
    public function updateInfos($userlogged, $infos, $id_hability){
        $a = "";
        $index = 1;
        ...
        $query->bindParam(":$this->pk", $id_hability);
        $index = 1;
        foreach ($infos as $key => $info) {
